...
def scrape():
    with sync_playwright() as p:
        ...
        # Fill login form
        print("Logging in...")
        page.fill('input[type="email"], input[name="email"], input[placeholder*="email" i]', GC_EMAIL)
        page.fill('input[type="password"], input[name="password"]', GC_PASSWORD)
...
def scrape_roster(page):
    ...
    try:
        ...
        players = []
        # Try multiple possible selectors GameChanger might use
        ...
        # Fallback: grab all text from the roster section
        if not players:
...
def scrape_results(page):
    ...
    try:
        # Results are usually on the schedule page filtered by completed
...
if __name__ == "__main__":
    ...
    print(json.dumps(data, indent=2)[:2000])  # Print first 2000 chars for debugging
